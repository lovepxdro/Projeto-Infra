...
def start_client(host='localhost', port=12345):
    global TIMEOUT, MAX_PAYLOAD_SIZE, WINDOW_SIZE, LOSS_PROBABILITY, ACK_LOSS_PROBABILITY, CORRUPTION_PROBABILITY, ACK_CORRUPTION_PROBABILITY, FORCE_LOST, FORCE_CORRUPT
    sem_perdas = "True"
    sem_corrupcoes = "True"
    ...
    try:
        ...
        if message.lower() != 'sair':
            ...
            force_lost_input = input("\nEnter - Simulação com perdas || -1 - Simulação sem perdas || \nInserção de qualquer outro nº - Pacote específico que gostaria de perder:  ")
            ...
            while True:
                ...
                force_lost_input = input("Enter - Continuar || Perder um pacote específico (Coloque o nº do pacote):  ")
                FORCE_LOST = set(map(int, force_lost_input.split(','))) if force_lost_input else set()
                
                force_corrupt_input = input("Enter - Continuar || Corromper um pacote específico (Coloque o nº do pacote):  ")
                FORCE_CORRUPT = set(map(int, force_corrupt_input.split(','))) if force_corrupt_input else set()
